// Prosty Panel — classicpanel.js (Wersja wydawnicza - zero błędów i logów)
...
function buildExtraStatus(host) {
    ...
    host._extraStatusCleanup = () => { if (recTimerId) { GLib.source_remove(recTimerId); recTimerId = 0; } };
    return box;
}
...
export const BottomTaskbar = GObject.registerClass({
    ...
    destroy() {
        ...
        // 🟢 BEZPIECZNE ROZŁĄCZANIE SYGNAŁÓW (FIX gsignal.c)
        for (const [obj, id] of this._signalIds) { 
            try { 
                if (obj === null) { GLib.source_remove(id); }
                else if (obj && typeof obj.disconnect === 'function') {
                    // Sprawdzamy czy handler nadal istnieje, by uniknąć ostrzeżeń w logach
